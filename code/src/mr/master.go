...
// Master 管理和调度任务，包括划分任务，管理任务状态
// 同时 Master 需要知道 Worker 的状态
// 队列机制使用 chanal 实现
...
func (m *Master) GetTaskHandler(args Args, reply *GetTaskReply) error {
	task := m.getTask()
	reply.Task = task
	reply.NReduce = m.nR
	reply.MMap = m.mM
...
func (m *Master) FinishTaskHandler(args Args, reply *FinishTaskReply) error {
	go func(task Task) {
		c := m.traceCh[task]
		c <- true
	}(args.Task)
...
func (m *Master) execPhase(phase string, taskQuantity int) {
	m.phase = phase
